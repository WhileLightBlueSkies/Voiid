...
//  Voiid
//
//  The privacy notice and the terms, as data, bundled in the app binary.
//
//  WHY BUNDLED AND NOT A WEB LINK
//  ------------------------------
//  DPDP s.5 requires the notice to be available at or BEFORE consent, and consent is
//  asked for on the very first screen of onboarding — before a phone number, before a
//  network round-trip, sometimes before a working connection. A notice that lives only on
//  a website is unreachable at exactly the moment it is legally required to be reachable,
//  and a link that 404s is worse than no link at all. So the text ships with the app and
//  renders natively. `database/migrations/031_consent_notice.sql` records this decision on
//  the registry row: its `url` is `app://voiid/legal/2026-08-01/en`, an honest locator for
//  a document that is genuinely here, rather than a plausible https:// URL that lies.
//
//  WHAT MUST STAY TRUE
//  -------------------
//  1. `noticeVersion` here, the `version` column of the seeded `consent_notices` row, and
//     `NOTICE_PURPOSES` in `backend/api/src/routes/consent.ts` are ONE version string in
//     three places. Change the words below and you have changed what people are agreeing
//     to: publish a NEW version in a migration and bump all three, never edit in place.
//     The server rejects a version it has not published, so a mismatch fails loudly at the
//     consent call rather than quietly recording agreement to text nobody saw.
//  2. The Android twin is
//     `apps/android/app/src/main/java/com/voiid/app/legal/LegalDocuments.kt`. The two
//     copies must say the same thing; a user on Android agreeing to "version 2026-08-01"
//     must have been shown the same words as a user on iOS.
//  3. Every factual claim below is a claim about code that exists. Where a behaviour is
//     stated but not yet built, it is listed under `pendingCounselOrBuild` and rendered to
//     the user as unfinished — not asserted in the body as if it were live.
//
...
//  named in `pendingCounselOrBuild` below so it is visible in the app instead of being a
//  gap nobody can see. The open questions are tracked in docs/LEGAL_QUESTIONS.md §3/§6 and
//  docs/research/11_admin_dpdp.md §6. Do not ship to the public App Store until that list
//  is empty.
//
...
/// One legal document, structured rather than a single blob of markdown: the renderer
/// needs headings it can style and anchor to, and a flat string cannot be checked against
/// the Android copy paragraph by paragraph.
struct LegalDocument: Identifiable, Hashable {
    let id: String
    let title: String
    /// The registry version this text belongs to. Rendered, because "which version did I
    /// agree to" must be answerable from the screen and not only from the database.
    let version: String
    let effectiveDate: String
    /// One-paragraph statement shown before the sections, in a heavier style.
    let summary: String
    let sections: [Section]
    /// Rendered under a "Still being finalised" heading. Empty means the document is done.
    let pendingCounselOrBuild: [String]

    struct Section: Identifiable, Hashable {
        let id: String
        let heading: String
        /// A paragraph, or — if it begins with "• " — a bullet. Keeping bullets in the same
        /// array preserves author order; a separate `bullets` field forces every section
        /// into "paragraphs then bullets", which is not how any of these read.
        let body: [String]
    }
}

// MARK: - The published version

enum LegalDocuments {
    /// MUST equal the `version` of the seeded `consent_notices` row and the key in the
    /// backend's `NOTICE_PURPOSES`. See the header.
    static let noticeVersion = "2026-09-10"
    /// BCP-47. Only English is published; whether the Eighth-Schedule translation
    /// obligation applies at Voiid's size is an open question for counsel, and seeding
    /// machine translations of a legal notice would answer it by accident.
    static let language = "en"

    static var all: [LegalDocument] { [privacy, terms] }

    // MARK: Purposes
    //
    // The display copy of `NOTICE_PURPOSES` in backend/api/src/routes/consent.ts. The
    // server is the authority on what may be stored — it rejects a key it does not
    // declare — and this is the copy shown to the person deciding. Both change together.

    struct Purpose: Identifiable, Hashable {
        let id: String
        let title: String
        let detail: String
        /// Required = Voiid cannot deliver the service without it. In this version every
        /// purpose is required, which is itself an open question (see the notice's
        /// pending list): DPDP s.6 wants consent that is specific and unconditional.
        let required: Bool
    }
    ...
    static let privacy = LegalDocument(
        ...
        summary: """
        Voiid cannot read your messages, hear your calls, see your live location or open your \
        ...
        """,
        sections: [
            .init(id: "event-payments", heading: "Community events and payments", body: [
    ...
    static let terms = LegalDocument(
        ...
        effectiveDate: "10 September 2026",
        summary: """
        The short version: your account is yours, the content you send is yours and we cannot \

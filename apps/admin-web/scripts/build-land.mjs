...
function ring(coords) {
  ...
  // A SMALL ring must not be simplified at the same tolerance as a continent. India's
  // Andaman, Nicobar and Lakshadweep groups are a few pixels across, and a 0.35px tolerance
  // collapsed each of them to a single point — which then rendered, under evenodd, as a
  // hole rather than an island. Scale the tolerance to the ring's own extent so a feature
  // can lose detail but never lose its shape.
  const xs = projected.map((p) => p[0]);
  const ys = projected.map((p) => p[1]);
  const extent = Math.max(Math.max(...xs) - Math.min(...xs), Math.max(...ys) - Math.min(...ys));
  const eps = Math.min(EPS, extent / 12);
...
function pathOf(geom) {
  ...
  return rings.flatMap(breakAtSeam)
              .map(ring)
              // Drop degenerate rings. Fewer than three distinct points encloses no area,
              // and under evenodd such a ring does not merely vanish — it can invert the
              // fill of the shape containing it.
...
// ── Jammu & Kashmir ──────────────────────────────────────────────────────────────
//
// A DELIBERATE SOVEREIGNTY CHOICE, not an accident of the data.
//
// Natural Earth's default view stops India at 35.5°N: Gilgit-Baltistan is drawn as
// Pakistan and Aksai Chin as China. India's official map includes the whole of the former
// princely state, and the Criminal Law (Amendment) Act 1990 requires maps published in
// India to show that boundary. Voiid is an Indian company and this panel is an Indian
// internal tool, so the map follows the Indian official boundary.
//
// The upstream shapes are single large polygons — Pakistan is one ring from 60.8°E — so
// the territory cannot be reassigned wholesale without handing India all of Pakistan.
// Instead the claimed area is drawn as an ADDITIONAL India polygon on top, and the
// underlying countries keep their own geometry beneath it. Painting order does the rest.
//
// The outline traces the claimed boundary of J&K including Gilgit-Baltistan and Aksai
// Chin. It is a simplified administrative boundary, at the same coarse resolution as
...
// Appended last so it paints ON TOP of the Pakistan and China polygons beneath it.
shapes.push({
  id: 'JK', region: 'India', name: 'Jammu & Kashmir',
  d: [JK_CLAIMED].map(ring).join(''),
});

...
alter table event_orders add column if not exists commission_bps integer check (commission_bps between 0 and 10000);
...
-- Guarded for the same reason as 066: replayed against a live database.
do $$ begin
...
create or replace function snapshot_event_commission() returns trigger language plpgsql as $$

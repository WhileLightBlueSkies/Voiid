...
export async function sweepMissedCallNotifications(deps = { query, send: sendFcmWake }): Promise<void> {
  const calls = await deps.query<{id:string;conversation_id:string;caller_user_id:string;call_kind:string}>(
    `with due as (
      ...
      order by c.started_at limit 50 for update of c skip locked
       ...
       status=case when c.status='ringing' then 'missed' else c.status end,
